...
const viewFields = ['id', 'organisation_id', 'name', 'timetable', 'number_routes', 'number_stops']
const viewLocationFields = ['mobile_id', 'current_stop_id', 'current_stop_departure', 'current_stop_name', 'previous_stop_id', 'previous_stop_departure', 'previous_stop_name', 'next_stop_id', 'next_stop_arrival', 'next_stop_name', 'updated', 'update_type', 'geox', 'geoy', 'route_section']
...
module.exports.getMobilesByOrganisationId = async (organisationId) => {
  let mobiles = []
  try {
    const query = 'select ' + viewFields.join(', ') + ' from vw_mobiles where organisation_id = $1'
...
module.exports.getMobileById = async (id) => {
  let mobile = null
  try {
    const query = 'select ' + viewFields.join(', ') + ' from vw_mobiles where id = $1'
    const params = [id]
...
module.exports.getMobileLocations = async () => {
  ...
  const getQuery = 'select ' + viewLocationFields.join(', ') + ' from vw_mobiles_location'
  try {
